...
extensions.listWebFrames = async function (useUrl = true) {
  ...
  if (this.isRealDevice() && this.remote && this.opts.bundleId) {
    ...
    const appInfo = await this.remote.connect();
    if (!appInfo) {
...
extensions.onPageChange = async function (pageChangeNotification) {
  ...
  } else if (!_.includes(ids, curPageIdKey)) {
  ...
  if (util.hasValue(this.curContext)) {
    const currentPageId = parseInt(_.last(this.curContext.split('.')), 10);
    const page = _.find(pageArray, (p) => parseInt(p.id, 10) === currentPageId);
